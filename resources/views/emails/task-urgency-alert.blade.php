...
@section('title', __('task.urgency_email_page_title'))
    ...
    @php
        ...
        if ($alert->task->due_date) {
            $detailRows[] = [
                ...
                'value' => DateFormatHelper::formatDateWithDayOfWeek($alert->task->due_date),
            ];
        }
    @endphp

    @include('emails.partials.task-notification-details', [
        'sectionTitle' => __('task.urgency_email_task_details'),
        'rows' => $detailRows,
    ])
